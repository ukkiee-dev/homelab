...
### 8.1 GHCR 사전 빌드 이미지 사용 (2026-04-19 이후)

- `ghcr.io/ukkiee-dev/postgres-backup:18-alpine-rclone<ver>` 에 rclone이 포함됨 — 런타임 다운로드 없음
- 빌드 파이프라인: `.github/workflows/build-postgres-backup.yml` (매월 1일 보안 패치 재빌드 + push 이벤트 trigger)
- Renovate가 Dockerfile의 `ARG RCLONE_V` 와 `FROM postgres:` tag를 자동 추적 → 신규 버전 PR 생성
- 이미지가 Private이므로 Pod spec에 `imagePullSecrets: [ghcr-pull-secret]` 필요 (apps namespace에 secret 선행 존재)

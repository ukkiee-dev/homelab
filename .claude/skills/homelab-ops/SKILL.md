...
| 에이전트 | 타입 | 모델 | 전문 영역 |
|---------|------|------|----------|
| `manifest-engineer` | general-purpose | opus | 매니페스트 생성/수정/검증, ArgoCD 정의 |
| `cluster-ops` | general-purpose | opus | 클러스터 진단, 트러블슈팅, 로그 분석 |
| `infra-reviewer` | general-purpose | opus | 인프라/보안/성능 종합 리뷰 |
...
#### 순차 실행 (앱 배포)
```
Phase 1: Agent(manifest-engineer) → 매니페스트 생성
Phase 2: Agent(infra-reviewer)   → 생성된 매니페스트 리뷰
Phase 3: 리뷰 피드백 있으면 → Agent(manifest-engineer) 재호출
```

#### 병렬 실행 (종합 감사)
```
Agent(cluster-ops, run_in_background=true, "클러스터 상태 진단: 노드/파드/리소스/이벤트")
Agent(infra-reviewer, run_in_background=true, "인프라 종합 리뷰: 보안/네트워킹/모니터링/ArgoCD")

---
name: homelab-ops
description: "K8s homelab 운영 오케스트레이터. 기존 앱 매니페스트 수정, 클러스터 진단, 일반 인프라 리뷰 등 빠른 운영 작업을 단일 에이전트(infra-reviewer)로 조율한다. 'homelab', '클러스터 운영', '진단해줘', '리뷰해줘', '점검해줘', '인프라 변경', '왜 안 되', '매니페스트 수정', '보안 확인', '리소스 늘려줘', '포트 바꿔줘' 등 운영·수정·진단 요청에 반응. 새 앱을 처음부터 설계·배포하거나 폐기하는 전체 라이프사이클 요청은 app-lifecycle 스킬이, 네트워크·시크릿·컨테이너·접근제어 4개 차원의 심층 보안 감사(Critical/High/Medium/Low 매트릭스 요청)는 infra-security-audit 스킬이 처리한다."
version: "1.0.0"
---
